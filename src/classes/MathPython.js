...
export var MathPY = {
    ...
    comb(n, k) {
        if (typeof n !== "number" || typeof k !== "number") throw new CalculError({ type: "SyntaxError", message: "Invalid Number" });
    ...
    ldexp(x, i) {
        if (typeof x !== 'number' || typeof i !== 'number') throw new CalculError({ type: 'SyntaxError', message: 'Invalid Number'})
        return (x * (2**i))
    },
